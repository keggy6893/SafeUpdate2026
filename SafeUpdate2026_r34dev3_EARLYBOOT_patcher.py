...
def patch_bootguard(text):
    ...
    if "set_startup_default" not in current_block:
        # Fresh r34-dev2 base: install the complete dev3 healthy handler.
        text = text[:start] + GUARD_REPLACEMENT + text[end:]
        changed = True
    else:
        # Existing r34-dev3 installation: upgrade the healthy-state JSON cleanup
        # without duplicating set_startup_default()/cleanup_source_startup_once().
        old_healthy = '''    plan["bootloader_fallback_armed"] = False
    plan["bootloader_default_slot"] = source
    plan["phase"] = "healthy"
    plan["healthy_at"] = int(time.time())
    save_plan(plan)
'''
        new_healthy = '''    now = int(time.time())
    plan["bootloader_fallback_armed"] = False
    plan["bootloader_default_slot"] = source
    plan["phase"] = "healthy"
    plan["healthy_at"] = now
    ...
    save_plan(plan)
'''
        if old_healthy in current_block:
            if text.count(old_healthy) != 1:
                die("safeupdate-bootguard: alter healthy-State-Block nicht eindeutig.")
            text = text.replace(old_healthy, new_healthy, 1)
            changed = True
        elif not (
            'plan["updated"] = now' in current_block and
            'plan.pop("fallback_reason", None)' in current_block and
            'plan.pop("error", None)' in current_block
        ):
            die("safeupdate-bootguard: dev3 healthy-State ist unbekannt; kein Blind-Patch.")
...
def main():
    ...
    print(" - Gesunder Abschluss bereinigt fallback_reason/error und aktualisiert updated")
    print("")
